  ...
  const load = async () => {
    ...
    try {
      ...
      const [jobRes, subRes] = await Promise.all([
        JobsAPI.getAll({ lat: coords.lat, lng: coords.lng, radius: 10, limit: 4 }),
        SubscriptionsAPI.getMy(),
      ])
      setRequests(jobRes.data.jobs)
  ...
  return (
    <div className="content-wrap">
      ...
      {sub && (
        <div className={`trial-bar fu ${sub.status === 'EXPIRED' ? 'expired' : ''}`}>
          <div>
            ...
            <p className="bsm t2">₦{sub.dailyRate}/day · Business Account</p>
          </div>
          {sub.status !== 'EXPIRED' && (
      ...
      <div className="stats-row fu fu1">
        <StatCard icon="📋" value={requests.length}            label="Service Requests" trend="Open now" />
        <StatCard icon="✅" value={user?.completedJobs ?? 0}  label="Fulfilled"        color="var(--gold2)" bg="rgba(200,146,10,.1)" />
      ...
      <div className="gr2 fu fu2 mb5">
        <div className="card card-grad cp" onClick={() => navigate('/business/requests')}
          ...
          <div><div className="tsm">Service Requests</div><div className="bsm t3">Browse incoming requests</div></div>
        </div>
        <div className="card card-grad cp" onClick={() => navigate('/business/nearby')}
          style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 18 }}>
          ...
          <div><div className="tsm">Nearby People</div><div className="bsm t3">See clients near your location</div></div>
        </div>
      </div>
      ...
      <div className="fu fu3">
        <div className="flex jb ac mb4">
          <h2 className="tmd">Service Requests Near You</h2>
          <button className="btn bts btn-sm" onClick={() => navigate('/business/requests')}>
            View All →
          </button>
        </div>
        {loading
          ...
          : requests.length
            ? requests.map(j => (
                <JobCard key={j._id} job={j} isWorker onAccepted={() => { refresh(); load() }} />
              ))
            : <Empty icon="🏪" title="No service requests nearby" sub="Check back soon or expand your radius." />
        }
      </div>
    </div>
  )
